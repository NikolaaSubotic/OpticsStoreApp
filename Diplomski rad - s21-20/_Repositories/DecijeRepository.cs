...
{
    public class DecijeRepository : BaseRepository, IDecijeRepository
    {

        public DecijeRepository(string connectionString)
        {
            this.connectionString = connectionString;
        }

        public void Add(DecijeModel decijeModel)
        {
            ...
            using (var command = new SqlCommand())
            {
                ...
                command.Parameters.Add("@ime", SqlDbType.NVarChar).Value = decijeModel.Ime;
                command.Parameters.Add("@boja", SqlDbType.NVarChar).Value = decijeModel.Boja;
                command.Parameters.Add("@cena", SqlDbType.Decimal).Value = decijeModel.Cena;
                command.Parameters.Add("@slika", SqlDbType.VarBinary).Value = decijeModel.Slika;
                // Ovde pristupite nazivu tipa iz vašeg modela
                string izabraniNazivTipa = decijeModel.TipNaočara; // Pretpostavljamo da je ovo naziv tipa.
        ...
        public void Edit(DecijeModel decijeModel)
        {
            using (var connection = new SqlConnection(connectionString))
            using (var command = new SqlCommand())
            {
                ...
                command.Parameters.Add("@tipId", SqlDbType.Int).Value = tipId;
                command.Parameters.Add("@id", SqlDbType.Int).Value = decijeModel.Id;
                command.ExecuteNonQuery();
            }
        }
        ...
        public IEnumerable<DecijeModel> GetAll()
        {
            var decijeList = new List<DecijeModel>();
            using (var connection = new SqlConnection(connectionString))
            using (var command = new SqlCommand())
            {
                connection.Open();
                ...
                command.CommandText = "SELECT n.*, t.Naziv AS TipNaočaraNaziv FROM Naocare n LEFT JOIN Tip t ON n.Tip_Id = t.Tip_Id WHERE n.Tip_Id = 3 ORDER BY n.Naocare_Id DESC";
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var decijeModel = new DecijeModel();
                        decijeModel.Id = (int)reader[0];
                        decijeModel.Ime = reader[1].ToString();
                        decijeModel.Boja = reader[2].ToString();
                        decijeModel.Cena = (decimal)reader[3];
                        if (reader[4] != DBNull.Value)
                        {
                            decijeModel.Slika = (byte[])reader[4]; // Učitavanje slike iz baze
                        }
                        else
                        {
                            decijeModel.Slika = null; // Postavite Slika svojstvo na null ako je NULL vrednost u bazi
                        }
                        // Postavite vrednost TipNaočara iz rezultata JOIN-a
                        decijeModel.TipNaočara = reader["TipNaočaraNaziv"].ToString();
                        decijeList.Add(decijeModel);
                    }
                }
            }
            return decijeList;
        ...
        public IEnumerable<DecijeModel> GetByValue(string value)
        {
            var naocareList = new List<DecijeModel>();
