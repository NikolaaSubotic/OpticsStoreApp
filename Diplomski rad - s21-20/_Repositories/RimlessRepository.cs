...
{
    public class RimlessRepository : BaseRepository, IRimlessRepository
    {

        public RimlessRepository(string connectionString)
        {
            this.connectionString = connectionString;
        }

        public void Add(RimlessModel rimlessModel)
        {
            ...
            using (var command = new SqlCommand())
            {
                ...
                command.Parameters.Add("@ime", SqlDbType.NVarChar).Value = rimlessModel.Ime;
                command.Parameters.Add("@boja", SqlDbType.NVarChar).Value = rimlessModel.Boja;
                command.Parameters.Add("@cena", SqlDbType.Decimal).Value = rimlessModel.Cena;
                command.Parameters.Add("@slika", SqlDbType.VarBinary).Value = rimlessModel.Slika;
                // Ovde pristupite nazivu tipa iz vašeg modela
                string izabraniNazivTipa = rimlessModel.TipNaočara; // Pretpostavljamo da je ovo naziv tipa.
        ...
        public void Edit(RimlessModel rimlessModel)
        {
            using (var connection = new SqlConnection(connectionString))
            using (var command = new SqlCommand())
            {
                ...
                command.Parameters.Add("@tipId", SqlDbType.Int).Value = tipId;
                command.Parameters.Add("@id", SqlDbType.Int).Value = rimlessModel.Id;
                command.ExecuteNonQuery();
            }
        }
        ...
        public IEnumerable<RimlessModel> GetAll()
        {
            var rimlessList = new List<RimlessModel>();
            using (var connection = new SqlConnection(connectionString))
            using (var command = new SqlCommand())
            {
                connection.Open();
                ...
                command.CommandText = "SELECT n.*, t.Naziv AS TipNaočaraNaziv FROM Naocare n LEFT JOIN Tip t ON n.Tip_Id = t.Tip_Id WHERE n.Tip_Id = 6 ORDER BY n.Naocare_Id DESC";
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var rimlessModel = new RimlessModel();
                        rimlessModel.Id = (int)reader[0];
                        rimlessModel.Ime = reader[1].ToString();
                        rimlessModel.Boja = reader[2].ToString();
                        rimlessModel.Cena = (decimal)reader[3];
                        if (reader[4] != DBNull.Value)
                        {
                            rimlessModel.Slika = (byte[])reader[4]; // Učitavanje slike iz baze
                        }
                        else
                        {
                            rimlessModel.Slika = null; // Postavite Slika svojstvo na null ako je NULL vrednost u bazi
                        }
                        // Postavite vrednost TipNaočara iz rezultata JOIN-a
                        rimlessModel.TipNaočara = reader["TipNaočaraNaziv"].ToString();
                        rimlessList.Add(rimlessModel);
                    }
                }
            }
            return rimlessList;
        ...
        public IEnumerable<RimlessModel> GetByValue(string value)
        {
            ...
            using (var command = new SqlCommand())
            {
                ...
                command.Parameters.Add("@boja", SqlDbType.NVarChar).Value = NaocareBoja;
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
